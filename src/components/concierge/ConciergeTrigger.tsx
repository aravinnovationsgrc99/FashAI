  ...
  return (
    <motion.button
      ...
      className={`fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-[250] flex items-center justify-center rounded-full bg-[#111111] transition-all duration-300 shadow-[0_6px_25px_rgba(0,0,0,0.3)] select-none group w-13 h-13 sm:w-14 sm:h-14 p-0 border ${
        isOpen
          ? "border-[#FAB60A] text-[#FAB60A] shadow-[0_0_25px_rgba(250,182,10,0.4)]"
          : "border-[#FAB60A]/70 text-white hover:border-[#F15E1C] hover:shadow-[0_0_30px_rgba(241,94,28,0.45)]"
      }`}
    >
      {!isOpen ? (
        ...
        <X className="w-5 h-5 text-[#FAB60A] relative z-10" />
      )}
    </motion.button>
  );
}
