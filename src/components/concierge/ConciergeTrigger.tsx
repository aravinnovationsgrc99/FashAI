...
export default function ConciergeTrigger({ isOpen, onToggle }: ConciergeTriggerProps) {
  return (
    <motion.button
      ...
      className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[250] flex items-center gap-3 px-3.5 py-2.5 sm:px-4 sm:py-2.5 bg-black/90 border rounded-full transition-colors duration-300 shadow-[0_0_30px_rgba(250,182,10,0.3)] select-none group min-h-[48px] ${
        isOpen
    ...
    >
      ...
      <div className="flex flex-col items-start text-left pr-1">
        <span className="font-syne text-[10px] sm:text-xs font-bold uppercase tracking-caps text-brand-white group-hover:text-brand-yellow-golden transition-colors duration-200 leading-none">
